...
static struct option long_options[] = {
	...
	{ _T("cpu"), 0, NULL, _T('c') },
	{ _T("device"), 1, NULL, _T('d') },
	{ _T("verbose"), 0, NULL, _T('v') },
	{ _T("input_file"),1 ,NULL, _T('i') },
	{ _T("print"),0,NULL, _T('p') },
	{ _T("affirm"),0,NULL, _T('a') },
	{ _T("repeat"),1,NULL, _T('r') },
	{ _T("kernel_file"),1,NULL, _T('k') },
	{ _T("wg_size"),1,NULL, _T('w') },
	{ _T("enqueue"),1,NULL, _T('e') },
	{ 0,0,0,0 }
};
...
size_t* default_wg_sizes(unsigned int* num_wg_sizes, const size_t max_wg_size, const size_t global_size)
{
	...
	if (wg_sizes == NULL)
		printf("csr.main() - Heap Overflow! Cannot allocate space for wg_sizes");
...
int main(int argc, TCHAR** argv)
{
	...
	unsigned int N = 512, num_execs = 1, i, ii, iii, j, k, num_wg_sizes = 0, num_kernels = 0;
	const unsigned int num_matrices = 1;
	unsigned long start_time, end_time;
	struct timeval *tv;
	TCHAR* file_path = NULL, *optptr;
	void* tmp;
	...
	FILE *kernel_fp;
	TCHAR *kernelSource,**kernel_files=NULL,*kernel_file_mode;

	//Does NOT use ocd_init() because we use TIMER_INIT (the 3rd thing in ocd_init) MANY TIMES in LOOOP! (nz-ocl)
	
	ocd_parse(&argc, (char***)&argv);
	ocd_check_requirements(NULL);
	
	ocd_initCL();
	
	
	while ((opt = getopt_long(argc, argv, _T("::vmw:k:i:par:::"), long_options, &option_index)) != -1 )
	{
		...
		//	break;
			...
			case 'r':
				if(optarg != NULL)
					num_execs = atoi((char*)optarg);
				else
					num_execs = atoi((char*)argv[optind]);
			...
			case 'w':
				...
				wg_sizes = (size_t*)tmp;
				wg_sizes[num_wg_sizes-1] = atoi((char*)optptr);
			break;
			default:
	...
	if(!file_path)
	{
		if (optarg != NULL) {
			...
			file_path = (TCHAR*)"C:\\Users\\ArvidJ\\Documents\\Visual Studio 2015\\Projects\\SPMV2\\Debug\\matrix-1536-5";//argv[optind];
	...
	do_affirm = 1;
	csr_matrix* csr = read_csr(&num_matrices,(char*)file_path);
	...
	for(ii=0; ii<num_matrices; ii++)
	{
		...
		device_out[ii] = (float*)float_new_array(csr[ii].num_rows,"csr.main() - Heap Overflow! Cannot Allocate Space for device_out");
		if(max_row_len < csr[ii].num_rows)
		{
			max_row_len = csr[ii].num_rows;
			y_host = (float*)float_array_realloc(y_host,csr[ii].num_rows,"csr.main() - Heap Overflow! Cannot Allocate Space for y_host");
		...
		if(max_col_len < csr[ii].num_cols)
		{
			...
			x_host = (float*)float_array_realloc(x_host,csr[ii].num_cols,"csr.main() - Heap Overflow! Cannot Allocate Space for x_host");
		}
	}
	...
	#ifdef ENABLE_TIMER
	tv = (timeval*)malloc(sizeof(struct timeval));
	...
	for(k=0; k<num_matrices; k++)
	{
		...
		if (_deviceType == 3){
			...
			csrCreateBuffer(&context,&csr_ap[k],sizeof(int)*(csr[k].num_rows+1), CL_MEM_READ_ONLY /*| CL_MEM_USE_HOST_PTR*/,"csr_ap",verbosity);
			csrCreateBuffer(&context,&x_loc[k],sizeof(float)*csr[k].num_cols, CL_MEM_READ_ONLY /*| CL_MEM_ALLOC_HOST_PTR*/,"x_loc",verbosity);
			csrCreateBuffer(&context,&y_loc[k],sizeof(float)*csr[k].num_rows, CL_MEM_READ_WRITE /*| CL_MEM_ALLOC_HOST_PTR*/,"y_loc",verbosity);
			csrCreateBuffer(&context,&csr_aj[k],sizeof(int)*csr[k].num_nonzeros, CL_MEM_READ_ONLY /*| CL_MEM_ALLOC_HOST_PTR*/,"csr_aj",verbosity);
			csrCreateBuffer(&context,&csr_ax[k],sizeof(float)*csr[k].num_nonzeros, CL_MEM_READ_ONLY /*| CL_MEM_ALLOC_HOST_PTR*/,"csr_ax",verbosity);
		}
	}
	...
	kernel_files = (TCHAR**)malloc(sizeof(char*)*num_kernels);
	kernel_files[0] = (TCHAR*)"spmv";
	...
	for(iii=0; iii<num_kernels; iii++) //loop through all kernels that need to be tested
	{
		...
		program = ocdBuildProgramFromFile(context,device_id,(char*)kernel_files[iii], NULL);
		...
		for(ii=0; ii<num_wg_sizes; ii++) //loop through all wg_sizes that need to be tested
		{
			...
			{						//to gather multiple samples of data
				...
				for(k=0; k<num_matrices; k++)
				{
					...
					/* Enqueue Kernel */
					wg_sizes[ii] = 256;
					std::cout << "WG_size: " << wg_sizes[ii] << std::endl;
				...
				clFinish(commands);
				clFinish(read_queue);
				cl_ulong time_start, time_end, write_start1, write_end1, write_start2, write_end2, write_start3, write_end3, write_start4, write_end4, write_start5, write_end5, read_start, read_end;
				clGetEventProfilingInfo(kernel_exec[0], CL_PROFILING_COMMAND_START, sizeof(time_start), &time_start, NULL);
				clGetEventProfilingInfo(kernel_exec[0], CL_PROFILING_COMMAND_END, sizeof(time_end), &time_end, NULL);

				clGetEventProfilingInfo(y_read[0], CL_PROFILING_COMMAND_START, sizeof(read_start), &read_start, NULL);
				clGetEventProfilingInfo(y_read[0], CL_PROFILING_COMMAND_END, sizeof(read_end), &read_end, NULL);

				clGetEventProfilingInfo(ap_write[0], CL_PROFILING_COMMAND_START, sizeof(write_start1), &write_start1, NULL);
				clGetEventProfilingInfo(ap_write[0], CL_PROFILING_COMMAND_END, sizeof(write_end1), &write_end1, NULL);

				clGetEventProfilingInfo(aj_write[0], CL_PROFILING_COMMAND_START, sizeof(write_start2), &write_start2, NULL);
				clGetEventProfilingInfo(aj_write[0], CL_PROFILING_COMMAND_END, sizeof(write_end2), &write_end2, NULL);

				clGetEventProfilingInfo(ax_write[0], CL_PROFILING_COMMAND_START, sizeof(write_start3), &write_start3, NULL);
				clGetEventProfilingInfo(ax_write[0], CL_PROFILING_COMMAND_END, sizeof(write_end3), &write_end3, NULL);

				clGetEventProfilingInfo(x_loc_write[0], CL_PROFILING_COMMAND_START, sizeof(write_start4), &write_start4, NULL);
				clGetEventProfilingInfo(x_loc_write[0], CL_PROFILING_COMMAND_END, sizeof(write_end4), &write_end4, NULL);

				clGetEventProfilingInfo(y_loc_write[0], CL_PROFILING_COMMAND_START, sizeof(write_start5), &write_start5, NULL);
				clGetEventProfilingInfo(y_loc_write[0], CL_PROFILING_COMMAND_END, sizeof(write_end5), &write_end5, NULL);
				...
				cl_ulong write_time5 = write_end5 - write_start5;
				cl_ulong total_write = write_time1 + write_time2 + write_time3 + write_time4 + write_time5;
				/*printf("OpenCl write time1 is: %0.6f \n", write_time1 / 1000000.0);
				printf("OpenCl write time2 is: %0.6f \n", write_time2 / 1000000.0);
				printf("OpenCl write time3 is: %0.6f \n", write_time3 / 1000000.0);
				printf("OpenCl write time4 is: %0.6f \n", write_time4 / 1000000.0);
				printf("OpenCl write time5 is: %0.6f \n", write_time5 / 1000000.0);*/

				cl_ulong total_time = kernel_time + read_time + write_time1 + write_time2 + write_time3 + write_time4 + write_time5;
				printf("OpenCl kernel Execution time is: %0.6f \n", kernel_time / 1000000.0);
				printf("OpenCl read time is: %0.6f \n", read_time / 1000000.0);
				printf("OpenCl write time is: %0.6f \n", total_write / 1000000.0);
				printf("OpenCl total time is: %0.6f \n", total_time / 1000000.0);
				#ifdef ENABLE_TIMER
				TIMER_STOP
				#endif
