                                 ...
                                 , const  char* args)
{
	...
	kernelLength = ((size_t)ftell(kernel_fp)-21);
	printf("kernelsize: %i \n",(size_t)ftell(kernel_fp)-21);
...
void* char_new_array(const size_t N,const char* error_msg)
{
	...
	int err;
	if (_deviceType == 3){
		ptr2 = _aligned_malloc(N * sizeof(char), ACL_ALIGNMENT);
		//err = posix_memalign(&ptr,ACL_ALIGNMENT,N * sizeof(char));
		check(!ptr2,error_msg);
	}
	else{
...
void* int_new_array(const size_t N,const char* error_msg)
{
	...
	int err;
	if (_deviceType == 3){
		ptr2 = _aligned_malloc(N * sizeof(int), ACL_ALIGNMENT);
...
void* long_new_array(const size_t N,const char* error_msg)
{
	...
	int err;
	if (_deviceType == 3){
		ptr2 = _aligned_malloc(N * sizeof(long), ACL_ALIGNMENT);
...
void* float_new_array(const size_t N,const char* error_msg)
{
	...
	int err;
	if (_deviceType == 3){
		ptr2 = _aligned_malloc(N * sizeof(float), ACL_ALIGNMENT);
