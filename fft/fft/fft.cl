...
__kernel void fft0(__global T2 *in, __global T2 *out)
{
	...
	__local T smem[2064];
	int i, j, r, Offset_in, Offset_out,  tid, b, d, k, l;
	int ii, jj, offset;// , fftn1, pow1; //placeholder
	T2 w;
	__local T *Local_store, *Local_load;
	T2 a[16];
	int local_id = get_local_id( 0 );
	int group_id = get_group_id( 0 );
	b = group_id & 2047;//(fftn1/2048-1);
	d = group_id >> 11;//(pow1-11);
	...
	i = tid & 15;
	j = tid >> 4;
	Offset_in += mad24(j, 128, i);//fftn1/
	in += Offset_in;
	globalLoads16(a, in ,16);//fftn1/
	FFT16(a);
	twiddle16(a, j, 128);
