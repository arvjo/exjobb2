...
//        << " " << id
    ...
    cout << cacheTypeValue << "\n";
    cout << localMemValue << "\n";
    cout << execCapabilitiesValue << "\n";
    cout << queuePropertiesValue << "\n";
...
//    iss >> (long)id
    ...
    string dummy;
    getline (cin, dummy);  // read the newline before the first string value
    
    // strings are one per line, \n is the separator
    for (i=0 ; i<numStringValues ; ++i)
    {
        getline (cin, stringValues[i]);
    }
    getline (cin, typeValue);
    getline (cin, addrBitsValue);
    getline (cin, cacheTypeValue);
    getline (cin, localMemValue);
    getline (cin, execCapabilitiesValue);
    getline (cin, queuePropertiesValue);

    if (hasHalfFp)
        getline (cin, halfFpValue);
    getline (cin, singleFpValue);
    if (hasDoubleFp)
        getline (cin, doubleFpValue);
}
