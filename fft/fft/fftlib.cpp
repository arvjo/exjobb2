...
forward(void* workp, void *temp, int n_ffts, int fftn)
{
	...
	clGetEventProfilingInfo(ocdTempEvent, CL_PROFILING_COMMAND_START, sizeof(time_start), &time_start, NULL);
	clGetEventProfilingInfo(ocdTempEvent, CL_PROFILING_COMMAND_END, sizeof(time_end), &time_end, NULL);
	total_time = time_end - time_start;

	printf("OpenCl Execution time is: %0.6f \n", total_time / 1000000.0);
...
copyToDevice(void* to_device, void* from_host, unsigned long bytes)
{
	...
	cout << "write time: " << write_time / 1000000.0  << endl;
...
copyFromDevice(void* to_host, void* from_device, unsigned long bytes)
{
	...
	cout << "read time: " << read_time / 1000000.0 << endl;
	printf("OpenCl total time is: %0.6f \n", (total_time + read_time + write_time) / 1000000.0);
