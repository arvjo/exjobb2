...
optscheck(option *options) {
	...
	opttest = (option**)calloc(256, sizeof(option *));
	if (!opttest) {
...
optsmake(char *str) {
	...
	ops = (option*)malloc((count + 1) * sizeof(option));
...
optsopt(int argc, char **argv, char *str) {
	...
	if (!last_optstring || strcmp(str, last_optstring) || !op) {
		if (last_optstring) {
			free(last_optstring);
			last_optstring = (char*)malloc(strlen(str) + 1);
	...
	if (!s) {
		...
		if (op->defalt) {
			...
			case OTYPE_STR:
				...
				C(op->value, char *) =
					(char*)malloc(strlen(C(op->defalt, char *)) + 1);
				if (C(op->value, char *))
